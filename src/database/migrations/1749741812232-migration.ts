...
export class Migration1749741812232 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE "${tableName}" ADD CONSTRAINT "${fk1Name}" FOREIGN KEY ("origin_workflow_id") REFERENCES "${tableName2}"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE "${tableName}" ADD CONSTRAINT "${fk2Name}" FOREIGN KEY ("target_workflow_id") REFERENCES "${tableName2}"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`);
    }
