...
import { PrometheusAsyncOptions, PrometheusOptions, PrometheusOptionsFactory, PrometheusOptionsWithDefaults } from './interfaces';
...
export class PrometheusModule {
  public static register<T extends RegistryContentType>(options?: PrometheusOptions<T>): DynamicModule {
  ...
  public static registerAsync<T extends RegistryContentType>(options: PrometheusAsyncOptions<T>): DynamicModule {
  ...
  public static createAsyncProviders<T extends RegistryContentType>(options: PrometheusAsyncOptions<T>): Provider[] {
    if (options.useExisting || options.useFactory) {
      return [this.createAsyncOptionsProvider(options), PrometheusModule.createPushgatewayProvider()];
  ...
  public static createAsyncOptionsProvider<T extends RegistryContentType>(options: PrometheusAsyncOptions<T>): Provider {
    ...
    return {
      provide: PROMETHEUS_OPTIONS,
      async useFactory<T extends RegistryContentType>(optionsFactory: PrometheusOptionsFactory<T>): Promise<PrometheusOptions<T>> {
  ...
  private static configureServer<T extends RegistryContentType>(options: PrometheusOptionsWithDefaults<T>): void {
  ...
  private static configurePushgateway<T extends RegistryContentType>(url: string, options?: unknown, registry?: promClient.Registry): promClient.Pushgateway<T> {
  ...
  private static makeDefaultOptions<T extends RegistryContentType>(options?: PrometheusOptions<T>): PrometheusOptionsWithDefaults<T> {
    return {
      global: false,
      path: '/metrics',
      defaultMetrics: {
