 ...
 * @internal
 */
export function getOrCreateMetric<T extends RegistryContentType = PrometheusContentType>(type: Metrics, options: Options, prometheusOptions?: PrometheusOptions<T>): client.Metric<string> {
  const opts: Options = {
    ...options,
    name: prometheusOptions?.customMetricPrefix ? prometheusOptions.customMetricPrefix.concat('_', options.name) : options.name,
  };
