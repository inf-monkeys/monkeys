...
export class TextExpansionController {
  ...
  public async textToImage(@Body() body: { text?: string; prompt?: string }) {
    ...
    const apiKey = readConfig('textExpansion.zaowuyunApiKey', process.env.ZAOWUYUN_API_KEY || 'sk-MPxsCh5vqc2DZAlcrKx3bpDTPJ8a7tOUnJaokouwJIFFJERs');
    if (!apiKey) {
      throw new HttpException('Zaowuyun API key not configured', HttpStatus.INTERNAL_SERVER_ERROR);
    }
