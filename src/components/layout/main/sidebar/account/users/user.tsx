...
export const User: React.FC<IUserProps> = ({ name, account, photo, isCollapsed }) => {
  return (
    <div className="flex items-center gap-2 py-1">
      <Avatar className={cn(isCollapsed && 'size-9')}>
      ...
      <div className="mt-0.5 flex flex-1 flex-col gap-1 [&_h1]:text-left [&_span]:text-left">
        {name ? <h1 className="line-clamp-1 font-bold leading-none">{name}</h1> : <Skeleton className="h-5 w-full" />}
        {account ? (
          <span className="line-clamp-1 text-xs leading-none text-black text-opacity-70 dark:text-gold-12 dark:opacity-70">
            {account}
          </span>
        ) : (
