...
func (s *Server) handleAssets(w http.ResponseWriter, r *http.Request) {
  ...
  if req.Name == "" || req.AssetType == "" || req.PrimaryContent == nil {
    writeJSON(w, http.StatusBadRequest, map[string]any{"error": "name, asset_type, primary_content required"})
    return
  }
...
func (s *Server) handleAssetByID(w http.ResponseWriter, r *http.Request) {
  ...
  case http.MethodPatch, http.MethodPut:
    ...
    if len(updates) == 0 && !hasTags {
      writeJSON(w, http.StatusBadRequest, map[string]any{"error": "no updates"})
      return
    }
    if len(updates) > 0 {
...
func (s *Server) handleTags(w http.ResponseWriter, r *http.Request) {
  ...
  case http.MethodGet:
    ...
    writeJSON(w, http.StatusOK, map[string]any{"items": items, "next_page_token": nextToken})
  default:
    writeJSON(w, http.StatusMethodNotAllowed, map[string]any{"error": "method not allowed"})
  }
}
...
func (s *Server) handleTagByID(w http.ResponseWriter, r *http.Request) {
  ...
  if id == "" {
    writeJSON(w, http.StatusBadRequest, map[string]any{"error": "invalid tag id"})
    return
  }
...
func (s *Server) handleViews(w http.ResponseWriter, r *http.Request) {
  ...
  if req.Name == "" || req.Path == "" {
    writeJSON(w, http.StatusBadRequest, map[string]any{"error": "name and path required"})
    return
  }
  now := nowMillis()
  if req.CreatedTimestamp == 0 {
...
func (s *Server) handleViewByID(w http.ResponseWriter, r *http.Request) {
  ...
  if id == "" {
    writeJSON(w, http.StatusBadRequest, map[string]any{"error": "invalid view id"})
    return
  }
