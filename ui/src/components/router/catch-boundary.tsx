...
export function ErrorComponent({ error }: ErrorBoundaryProps) {
  ...
  return (
    ...
    >
      <div className="text-black-500 flex items-center gap-2 font-bold">
        <h1 className="text-black-10 leading-tight">{t('system.error.title')}</h1>
      </div>
        ...
        <div className="max-w-full overflow-hidden rounded bg-gray-10 bg-opacity-10 p-2 backdrop-blur-sm">
          <Collapsible defaultOpen>
            <CollapsibleTrigger asChild>
              <p className="text-black-500 text-sm">{t('system.error.show-detail')}</p>
            </CollapsibleTrigger>
            <CollapsibleContent>
      ...
      <TooltipProvider delayDuration={100}>
        <div className="flex items-center gap-2">
          ...
          <Button size="small" variant="outline" onClick={() => setShowDetails((prev) => !prev)}>
