...
const VinesImage: React.FC<IVinesImageProps> = memo(
  ({ src, alt = 'image', className, style, imageStyle, disabledPreview }) => {
    const { ref, height, width } = useElementSize();
    const [inViewport] = useInViewport(ref);

    return (
      <div ref={ref} className={cn('relative flex items-center justify-center', className)} style={style}>
        <AnimatePresence>
          {inViewport && (
            <LazyImage
              src={src}
