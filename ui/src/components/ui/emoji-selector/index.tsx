...
interface IVinesEmojiSelectorProps extends Omit<React.ComponentPropsWithoutRef<'div'>, 'onChange'> {
  emojiLink?: string;
...
export const VinesEmojiSelector: React.FC<IVinesEmojiSelectorProps> = memo(
        ...
        >
          ...
          <div className="flex h-20 w-full items-center justify-between gap-2 px-5 pb-1">
            <div className={onlyEmoji ? 'hidden' : ''}>
              <p className="mb-2 text-sm">{t('components.ui.emoji-selector.background-color')}</p>
...
VinesEmojiSelector.displayName = 'VinesEmojiSelector';
