...
export function lagRadar(config: ILagRadarConfig = {}) {
  ...
  const calcHue = (() => {
    ...
    return (ms_delta: number) => max_hue - Math.max(0, Math.min(mult * Math.log(ms_delta / log_f), max_hue));
  })();
  ...
  function animate() {
    ...
    for (let i = 0; i < frames; i++) {
      $arcs[(frames + framePtr - i) % frames].style.fillOpacity = String(1 - i / frames);
    }
