...
interface IAgentConfigFormFieldKnowledgeBaseProps {
  form: UseFormReturn<IAgentConfig>;
}

export const AgentConfigFormFieldKnowledgeBase: React.FC<IAgentConfigFormFieldKnowledgeBaseProps> = ({ form }) => {
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="flex-1">
                <FormLabel>{t('agent.view-config.form.knowledge-base.label')}</FormLabel>
                <FormControl>
                  <Select onValueChange={field.onChange} value={field.value}>
                    <FormControl>
                      <SelectTrigger>
                        <SelectValue placeholder={t('agent.view-config.form.knowledge-base.placeholder')} />
                      </SelectTrigger>
                    </FormControl>
                    <SelectContent>
                      ...
                      {isEmptyList && (
                        <SelectItem disabled value="empty">
                          {t('agent.view-config.form.knowledge-base.empty')}
                        </SelectItem>
                      )}
                    </SelectContent>
                  </Select>
