...
export type GarmentOnModelOptions = InspirationGenerationOptions;

export const GarmentOnModelPanel: React.FC<{ options?: GarmentOnModelOptions }> = ({ options }) => {
  ...
  const handleStart = async () => {
    ...
    const clothSlot = referenceSlots.find((s) => s.id === 'reference');
    ...
    try {
      ...
      const inputData: Record<string, unknown> = {
        hgbbnh: '趋势模型',
        image_model: uploadMap['base'] ?? '',
        image_cloth: uploadMap['reference'] ?? '',
      };
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto pb-2 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
          <PanelCard className="gap-4" padding={12}>
            <PanelHeader
              ...
              title={resolvedOptions.title ?? '成衣上身'}
              description="上传模特图和款式图（可选）"
            />
        ...
        <div className="flex w-full items-center justify-between gap-3 pb-1 pt-1">
          <button
            ...
            disabled={
              ...
              !referenceSlots.find((s) => s.id === 'reference' && s.uploadedUrl)
            }
          >
