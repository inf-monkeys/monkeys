...
export type LineToGarmentOptions = InspirationGenerationOptions;

export const LineToGarmentPanel: React.FC<{ options?: LineToGarmentOptions }> = ({ options }) => {
  ...
  const [referenceSlots, setReferenceSlots] = useState<ReferenceSlot[]>([
    { id: 'base', label: '线稿图' },
    { id: 'reference', label: '面料图（可选）', optional: true },
  ]);
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      const inputData: Record<string, unknown> = {
        Line_drawing: uploadMap['base'] ?? '',
        reference_image: uploadMap['reference'] ?? '',
        Prompt: prompt ?? '',
        seed: randomSeed15(),
        steps: 25,
        strength: '1',
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto pb-2 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
          <PanelCard className="gap-4" padding={12}>
            <PanelHeader
              ...
              title={resolvedOptions.title ?? '线稿成衣'}
              description="上传线稿图和可选的面料图"
            />
