...
export type SmartOutfitOptions = InspirationGenerationOptions & {
  ratioOptions?: string[];
};

export const SmartOutfitPanel: React.FC<{ options?: SmartOutfitOptions }> = ({ options }) => {
  ...
  const [styleValue, setStyleValue] = useState<string>('巴恩风');
  const [poseValue, setPoseValue] = useState<string>('自然直立');
  const [prompt, setPrompt] = useState(resolvedOptions.prompt ?? '');
  const [ratioValue, setRatioValue] = useState<string>(() => resolvedOptions.ratioOptions?.[0] ?? '1:1');

  const [starting, setStarting] = useState(false);
  const [uploading, setUploading] = useState(false);
  const [referenceSlots, setReferenceSlots] = useState<ReferenceSlot[]>([
    { id: 'cloth', label: '服装图' },
    { id: 'model', label: '模特图（可选）', optional: true },
  ]);

  const ratioOptions = resolvedOptions.ratioOptions ?? ['1:1', '16:9', '9:16', '4:3', '3:4'];
  const styleOptions = ['巴恩风', '运动风', '自定义'];
  const poseOptions = ['自然直立', '侧面直立', '坐姿', '自定义'];
  ...
  const handleStart = async () => {
    ...
    const clothSlot = referenceSlots.find((s) => s.id === 'cloth');
    if (!clothSlot?.file && !clothSlot?.uploadedUrl) {
      toast.error('请先上传服装图');
      return;
    }
    if (!ratioValue) {
    ...
    try {
      ...
      const inputData: Record<string, unknown> = {
        '7q8jzd': styleValue.trim(),
        dgmckm: ratioValue,
        image_cloth: uploadMap['cloth'] ?? '',
        image_model: uploadMap['model'] ?? '',
        prompt_input: prompt ?? '',
        rqpztw: poseValue.trim(),
      };
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto pb-2 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
          <PanelCard className="gap-4" padding={12}>
            <PanelHeader
              ...
              title={resolvedOptions.title ?? '智能穿搭'}
              description="上传服装图与模特图（可选）"
            />
          ...
          <PanelCard className="gap-4" padding={12}>
            <PanelHeader icon={<BsdLightIcon className="size-5" />} title="创意描述" description="填写风格与姿势参考，可选提示词" />
            <div className="flex flex-col gap-3">
              <div className="grid grid-cols-2 gap-3">
                <div className="flex flex-col gap-2">
                  <p className="text-sm text-white/70">风格</p>
                ...
                <div className="flex flex-col gap-2">
                  <p className="text-sm text-white/70">姿势参考</p>
                  <Select value={poseValue} onValueChange={setPoseValue}>
                    <SelectTrigger
                      className="flex h-9 w-full items-center rounded-2xl border border-white/20 px-4 text-left text-sm text-white transition hover:border-white/30 focus:border-white/40 focus:ring-0 [&>span]:flex [&>span]:h-full [&>span]:w-full [&>span]:items-center [&>span]:font-['Noto_Sans_SC'] [&>span]:text-[14px] [&>span]:font-normal [&>span]:leading-[140%] [&>span]:tracking-[0em] [&>span]:text-white [&>span[data-placeholder]]:text-white/50"
                      style={{ background: 'rgba(44, 94, 245, 0.15)' }}
                    >
                      <SelectValue placeholder="选择姿势参考" />
                    </SelectTrigger>
                    <SelectContent className="border-white/10 bg-slate-900/95 text-white">
                      {poseOptions.map((opt) => (
                        <SelectItem key={opt} value={opt} className="text-sm">
              ...
              >
                <textarea
                  ...
                  placeholder="请输入提示词（可留空）"
                />
              </div>
            </div>
          </PanelCard>

          <RatioSelector value={ratioValue} options={ratioOptions} onChange={setRatioValue} />
        </div>
          ...
          <button
            ...
            style={{
              ...
              disabled={
                ...
                uploading ||
                !workflowId ||
                !referenceSlots.find((s) => s.id === 'cloth' && s.uploadedUrl) ||
                !ratioValue ||
                !styleValue.trim() ||
                !poseValue.trim()
              }
