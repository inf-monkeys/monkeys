...
export type AtmosphereOptions = InspirationGenerationOptions & {
  typeOptions?: string[];
  styleOptions?: string[];
  ratioOptions?: string[];
};

export const AtmospherePanel: React.FC<{ options?: AtmosphereOptions }> = ({ options }) => {
  ...
  const [typeValue, setTypeValue] = useState<string>(() => resolvedOptions.typeOptions?.[0] ?? '风格图');
  const [styleValue, setStyleValue] = useState<string>(() => resolvedOptions.styleOptions?.[0] ?? '自然户外');
  ...
  const [referenceSlots, setReferenceSlots] = useState<ReferenceSlot[]>([{ id: 'base', label: '图像参考（可选）', optional: true }]);

  const typeOptions = resolvedOptions.typeOptions ?? ['风格图'];
  const styleOptions = resolvedOptions.styleOptions ?? ['自然户外', '都市运动', '都市生活', '都市游牧', '极地严寒', '自定义'];
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      const inputData: Record<string, unknown> = {
        ...
        'bb7tjq': styleValue,
        'bgcwkf': ratioValue,
        'zjkbj6': prompt?.trim() ? prompt.trim() : typeValue,
        '9dtkfh': uploadMap['base'] ?? '',
      };
      ...
      <div className="flex w-[400px] min-h-0 flex-col gap-3">
        <div className="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto pb-2 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
          <PanelCard className="gap-4" padding={12}>
            <PanelHeader
              ...
              title={resolvedOptions.title ?? '氛围图'}
              description="上传图像参考（可选）并选择类型、风格、比例"
            />
          ...
          <PanelCard className="gap-4" padding={12}>
            <PanelHeader
              ...
              title="创意描述"
              description="选择类型/风格，可填写提示词"
            />
            <div className="flex flex-col gap-3">
              <div className="grid grid-cols-2 gap-3">
                <div className="flex flex-col gap-2">
                  <p className="text-sm text-white/70">类型</p>
        ...
        <div className="flex w-full items-center justify-between gap-3 pb-1 pt-1">
          <button
            ...
            disabled={starting || uploading || !workflowId || !typeValue || !styleValue || !ratioValue}
          >
