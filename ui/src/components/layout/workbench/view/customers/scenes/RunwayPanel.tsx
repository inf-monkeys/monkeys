...
export type RunwayOptions = InspirationGenerationOptions & {
  ratioOptions?: string[];
  categoryOptions?: string[];
  angleOptions?: string[];
};

export const RunwayPanel: React.FC<{ options?: RunwayOptions }> = ({ options }) => {
  ...
  const categoryOptions = resolvedOptions.categoryOptions ?? ['羽绒服', '冲锋衣', '功能外套'];
  const angleOptions = resolvedOptions.angleOptions ?? ['正视', '侧视'];
  ...
  const [angle, setAngle] = useState<string>(angleOptions[0] ?? '');
  const [prompt, setPrompt] = useState(resolvedOptions.prompt ?? '');
  const [ratioValue, setRatioValue] = useState<string>(ratioOptions[0] ?? '1:1');

  const [starting, setStarting] = useState(false);
  const [uploading, setUploading] = useState(false);
  const [referenceSlots, setReferenceSlots] = useState<ReferenceSlot[]>([{ id: 'reference', label: '参考图（可选）', optional: true }]);
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      const fallbackPrompt = prompt?.trim() ? prompt.trim() : `${category}，${angle}`;
      const inputData: Record<string, unknown> = {
        ddgctn: category,
        qpdhwn: angle,
        ngckqk: ratioValue,
        thtqbh: uploadMap['reference'] ?? '',
        zjkbj6: fallbackPrompt,
      };
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto pb-2 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
          <PanelCard className="gap-4" padding={12}>
            <PanelHeader
              ...
              title={resolvedOptions.title ?? '秀场款生成'}
              description="上传参考图（可选）"
            />
          ...
          <PanelCard className="gap-4" padding={12}>
            <PanelHeader icon={<BsdLightIcon className="size-5" />} title="创意描述" description="选择品类与角度，可填写提示词" />
            <div className="flex flex-col gap-3">
              <div className="grid grid-cols-2 gap-3">
                <div className="flex flex-col gap-2">
                  <p className="text-sm text-white/70">品类</p>
                ...
                <div className="flex flex-col gap-2">
                  <p className="text-sm text-white/70">角度</p>
