...
import { OAuthProvider } from '@/components/layout/login/auth/oauth/typing';
...
export const ArtistLogin: React.FC<IArtistLoginProps> = ({
  ...
  const oauthProviders = loginMethods
    .map((method) => (typeof method === 'string' && method.startsWith('oauth-') ? method.replace('oauth-', '') : ''))
    .filter(Boolean);
            ...
            <div className="mt-4 w-full flex flex-col items-center gap-2">
              ...
              <div className="flex flex-wrap items-center justify-center gap-2 w-full">
                {oauthProviders.map((providerName, i) => {
                  ...
                  if (!OAProvider) return null;
                  const isDingtalk = providerName === 'dingtalk';
                  return (
                    <OAProvider.Provider key={i}>
                      <Button
                        type="button"
                        className="dingtalk-button flex items-center justify-center gap-2"
                        style={{
                          width: '50%',
                          maxWidth: '200px',
                          minWidth: '120px',
                      ...
                      >
                        {isDingtalk ? (
                          ...
                          OAProvider.icon && <OAProvider.icon className="h-5 w-5" />
                        )}
