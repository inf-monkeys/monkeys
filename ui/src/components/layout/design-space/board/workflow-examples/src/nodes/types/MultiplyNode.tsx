  ...
  getPorts(_shape: NodeShape, _node: MultiplyNode): Record<string, ShapePort> {
    return {
      ...
      multiplier: {
        id: 'multiplier',
        x: 0,
        y: NODE_HEADER_HEIGHT_PX + NODE_ROW_HEADER_GAP_PX + NODE_ROW_HEIGHT_PX + NODE_ROW_HEIGHT_PX / 2,
        terminal: 'end',
      },
    };
  }
  async execute(shape: NodeShape, node: MultiplyNode, inputs: InputValues): Promise<ExecutionResult> {
    await sleep(1000);
  ...
  return (
    <>
      <NodeInputRow
        ...
        portId="multiplicand"
        value={node.a}
        onChange={(newValue) => updateNode<MultiplyNode>(editor, shape, (node) => ({ ...node, a: newValue }))}
      />
      <NodeInputRow
        shapeId={shape.id}
        portId="multiplier"
        value={node.b}
        onChange={(newValue) => updateNode<MultiplyNode>(editor, shape, (node) => ({ ...node, b: newValue }))}
      />
    </>
  );
}
