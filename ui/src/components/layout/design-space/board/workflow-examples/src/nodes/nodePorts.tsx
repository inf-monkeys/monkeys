 ...
 */
export function getNodePortConnections(editor: Editor, shape: NodeShape | TLShapeId): NodePortConnection[] {
    ...
    if (Object.values(inputs).some((input) => input.value === STOP_EXECUTION)) {
      return Object.fromEntries(
        Object.values(getNodePorts(editor, node))
          ...
          .map((port) => [port.id, { value: STOP_EXECUTION, isOutOfDate: areAnyInputsOutOfDate(inputs) }]),
      );
    }
 ...
 */
export function getAllConnectedNodes(editor: Editor, startingNode: TLShapeId | NodeShape, direction?: 'start' | 'end') {
