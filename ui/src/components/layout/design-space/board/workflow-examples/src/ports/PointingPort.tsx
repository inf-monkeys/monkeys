...
export class PointingPort extends StateNode {
  ...
  override onPointerMove(info: TLPointerEventInfo): void {
    ...
    if (this.editor.inputs.isDragging) {
      ...
      const handle = this.editor.getShapeHandles(connectionShapeId)?.find((h) => h.id === draggingTerminal);
  ...
  private onClick() {
    ...
    onCanvasComponentPickerState.set(this.editor, {
      ...
      onClose: () => {
        ...
        if (!connection || !this.editor.isShapeOfType<ConnectionShape>(connection, 'connection')) return;
