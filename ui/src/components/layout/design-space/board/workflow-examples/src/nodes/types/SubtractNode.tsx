...
export class SubtractNodeDefinition extends NodeDefinition<SubtractNode> {
  ...
  async execute(shape: NodeShape, node: SubtractNode, inputs: InputValues): Promise<ExecutionResult> {
    await sleep(1000);
  ...
  return (
    <>
      <NodeInputRow
        ...
        portId="minuend"
        value={node.a}
        onChange={(newValue) => updateNode<SubtractNode>(editor, shape, (node) => ({ ...node, a: newValue }))}
      />
      <NodeInputRow
        shapeId={shape.id}
        portId="subtrahend"
        value={node.b}
        onChange={(newValue) => updateNode<SubtractNode>(editor, shape, (node) => ({ ...node, b: newValue }))}
      />
    </>
  );
}
