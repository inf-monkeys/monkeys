        ...
        Object.keys(node.items)
          ...
          .map((idx, i) => [
            `item_${idx}`,
            {
              id: `item_${idx}`,
              x: 0,
              y: NODE_HEADER_HEIGHT_PX + NODE_ROW_HEADER_GAP_PX + NODE_ROW_HEIGHT_PX * i + NODE_ROW_HEIGHT_PX / 2,
              terminal: 'end',
            },
          ]),
      ),
...
function ensureFinalEmptyItem(
  ...
  if (removeUnused) {
    entriesToKeep = entriesToKeep.filter(([idx, value], i) => {
      ...
      return i === 0 || i === entriesToKeep.length - 1 || value !== 0 || connectedPortIds.has(portId);
    });
