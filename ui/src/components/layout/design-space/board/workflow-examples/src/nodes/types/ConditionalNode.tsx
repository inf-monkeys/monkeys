...
export class ConditionalNodeDefinition extends NodeDefinition<ConditionalNode> {
  ...
  async execute(shape: NodeShape, node: ConditionalNode, inputs: InputValues): Promise<ExecutionResult> {
    await sleep(1000);
  ...
  getOutputInfo(shape: NodeShape, node: ConditionalNode, inputs: InfoValues): InfoValues {
    ...
    return {
      outputTrue: node.previousResult === 'rhs' ? { value: STOP_EXECUTION, isOutOfDate } : { value: lhs, isOutOfDate },
      outputFalse: node.previousResult === 'rhs' ? { value: rhs, isOutOfDate } : { value: STOP_EXECUTION, isOutOfDate },
    };
  }
  ...
  return (
    <>
      <NodeInputRow
        ...
        portId="lhs"
        value={node.lhs}
        onChange={(newValue) => updateNode<ConditionalNode>(editor, shape, (node) => ({ ...node, lhs: newValue }))}
      />
      <NodeRow>
        <select
          value={node.operator}
      ...
      <NodeInputRow
        ...
        portId="rhs"
        value={node.rhs}
        onChange={(newValue) => updateNode<ConditionalNode>(editor, shape, (node) => ({ ...node, rhs: newValue }))}
      />
