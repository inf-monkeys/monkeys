...
function NodeShapeIndicator({ shape, ports }: { shape: NodeShape; ports: ShapePort[] }) {
  ...
  return (
    <>
      ...
      <mask id={id}>
        <rect width={NODE_WIDTH_PX + 10} height={getNodeHeightPx(editor, shape) + 10} fill="white" x={-5} y={-5} />
        {ports.map((port) => (
          <circle key={port.id} cx={port.x} cy={port.y} r={PORT_RADIUS_PX} fill="black" strokeWidth={0} />
        ))}
      </mask>
      <rect rx={9} width={NODE_WIDTH_PX} height={getNodeHeightPx(editor, shape)} mask={`url(#${id})`} />
      {ports.map((port) => (
...
function NodeShape({ shape }: { shape: NodeShape }) {
  ...
  const output = useValue('output', () => getNodeOutputPortInfo(editor, shape.id)?.output ?? undefined, [
    editor,
    shape.id,
  ]);
