...
export const DesignBoardRightSidebar: React.FC<RightSidebarProps> = ({
  ...
  const mapHexToTldrawColor = (hex: string): string => {
    ...
    const distances = Object.entries(colorMap).map(([hexColor, colorName]) => {
      ...
      const distance = Math.sqrt(
        Math.pow(r - colorRgb.r, 2) + Math.pow(g - colorRgb.g, 2) + Math.pow(b - colorRgb.b, 2),
      );
      return { colorName, distance };
    });

    // 返回距离最小的颜色
    return distances.reduce((min, current) => (current.distance < min.distance ? current : min)).colorName;
  };

  // 应用填充颜色到选中的形状
  const applyFillToSelection = React.useCallback(
    (fillColor: string) => {
      if (!editor) return;
  ...
  const applyOpacityToSelection = React.useCallback(
    (pct: number) => {
      if (!editor) return;
      const e: any = editor as any;
      ...
      ids.forEach((id: any) => {
        ...
        if (typeof s.props === 'object' && typeof s.props.opacity === 'number')
          next.props = { ...s.props, opacity: value01 };
  ...
  const applyCornerToSelection = React.useCallback(
    (radius: number) => {
      if (!editor) return;
      const e: any = editor as any;
      const ids = e.getSelectedShapeIds?.() || [];
  ...
  const handleAlign = (type: 'left' | 'center-horizontal' | 'right' | 'top' | 'center-vertical' | 'bottom') => {
    if (!editor) return;
    const e: any = editor as any;
    const exec = () => {
  ...
  const handleZIndex = (action: 'bringToFront' | 'bringForward' | 'sendBackward' | 'sendToBack') => {
    if (!editor) return;
    const e: any = editor as any;
    const exec = () => {
      const ids = e.getSelectedShapeIds?.() || [];
                ...
                {!hasSelection ? (
                  <div className="flex h-full flex-col">
                    ...
                    <div className="sticky bottom-0 left-0 right-0">
                      <Button className="w-full" variant="outline" onClick={handleExportAll}>
                        导出全部
                      </Button>
                    </div>
                  </div>
                  ...
                  <>
                    <TabsContent value="design" className="mt-0">
                      <div className="flex flex-col gap-3">
                        ...
                        <section>
                          <h1 className="mb-2 mt-2 text-sm font-semibold">位置</h1>
                          ...
                          <div className="mb-2 flex items-center gap-1">
                            <Button
                              ...
                              title="左对齐"
                              disabled={!canAlign}
                              onClick={() => canAlign && handleAlign('left')}
                            >
                              <AlignLeft size={16} />
                            </Button>
                            <Button
                              ...
                              title="水平居中"
                              disabled={!canAlign}
                              onClick={() => canAlign && handleAlign('center-horizontal')}
                            >
                            ...
                            <Button
                              ...
                              title="右对齐"
                              disabled={!canAlign}
                              onClick={() => canAlign && handleAlign('right')}
                            >
                              <AlignRight size={16} />
                            </Button>
                            ...
                            <Button
                              ...
                              title="上对齐"
                              disabled={!canAlign}
                              onClick={() => canAlign && handleAlign('top')}
                            >
                              <ArrowUp size={16} />
                            </Button>
                            <Button
                              ...
                              title="垂直居中"
                              disabled={!canAlign}
                              onClick={() => canAlign && handleAlign('center-vertical')}
                            >
                            ...
                            <Button
                              ...
                              title="下对齐"
                              disabled={!canAlign}
                              onClick={() => canAlign && handleAlign('bottom')}
                            >
                              <ArrowDown size={16} />
                            </Button>
                            ...
                            <div className="flex flex-col gap-1">
                              ...
                              <input
                                ...
                                onBlur={() => {
                                  ...
                                  if (!dx) return;
                                  const ids = editor.getSelectedShapeIds?.() || [];
                                  if (typeof e.nudgeSelectedShapes === 'function')
                                    return e.nudgeSelectedShapes({ x: dx, y: 0 });
                            ...
                            <div className="flex flex-col gap-1">
                              ...
                              <input
                                ...
                                onBlur={() => {
                                  ...
                                  const ids = editor.getSelectedShapeIds?.() || [];
                                  if (typeof e.nudgeSelectedShapes === 'function')
                                    return e.nudgeSelectedShapes({ x: 0, y: dy });
                        ...
                        <section>
                          ...
                          <div className="mb-2 flex items-center gap-1">
                            <Button
                              ...
                              variant="ghost"
                              title="上移一层"
                              disabled={!canZIndex.bringForward}
                              onClick={() => canZIndex.bringForward && handleZIndex('bringForward')}
                            >
                              <ChevronUp size={16} />
                            </Button>
                            <Button
                              ...
                              variant="ghost"
                              title="下移一层"
                              disabled={!canZIndex.sendBackward}
                              onClick={() => canZIndex.sendBackward && handleZIndex('sendBackward')}
                            >
                              <ChevronDown size={16} />
                            </Button>
                            <Button
                              ...
                              variant="ghost"
                              title="移到最顶"
                              disabled={!canZIndex.bringToFront}
                              onClick={() => canZIndex.bringToFront && handleZIndex('bringToFront')}
                            >
                            ...
                            <Button
                              ...
                              variant="ghost"
                              title="移到最低"
                              disabled={!canZIndex.sendToBack}
                              onClick={() => canZIndex.sendToBack && handleZIndex('sendToBack')}
                            >
                            ...
                            {fillPickerOpen &&
                              ...
                              createPortal(
                                <div
                                  ...
                                  style={{
                                    position: 'fixed',
                                    top: fillPanelPos.top,
                                    left: fillPanelPos.left,
                                    width: 240,
                                    height: 420,
                                    zIndex: 9999,
                                  }}
                                    ...
                                    <div className="mt-4 space-y-3">
                                      <div className="flex items-center gap-1">
                                        <div className="relative">
                                          ...
                                          <ChevronDown className="pointer-events-none absolute right-0.5 top-1/2 h-2.5 w-2.5 -translate-y-1/2 text-neutral-500" />
                                        </div>
                                        <input
                                          className="h-8 w-24 border border-neutral-200 bg-white px-1.5 font-mono text-xs dark:border-neutral-600 dark:bg-neutral-700 dark:text-white"
                                          style={{ borderRadius: '5px' }}
                                          value={getColorValue()}
                                          readOnly
                                        />
                            ...
                            <div className="flex flex-col gap-2 text-xs">
                              ...
                              {fills.map((c, idx) => (
                                <div key={idx} className="flex items-center gap-2 rounded border p-2">
                                  <div
                                    className="h-4 w-4 cursor-pointer rounded-sm border transition-transform hover:scale-110"
                                    style={{ background: c }}
                                  ...
                                  <Button
                                    size="small"
                                    variant="ghost"
                                    onClick={() => setFills((l) => l.filter((_, i) => i !== idx))}
                                  >
                                    删除
                                  </Button>
                                </div>
