...
export const MiniHistoryRightSidebar: React.FC<MiniHistoryRightSidebarProps> = ({
  ...
  const [pendingHistory, setPendingHistory] = React.useState<
  ...
  React.useEffect(() => {
    const onCreated = (e: any) => {
      ...
      setPendingHistory((list) => [
        { instanceId, workflowId: wf, time: Date.now() },
  ...
  const renderHistory = () => {
    try {
      ...
      const waitingList = (pendingHistory || []).filter((p) => p.workflowId === workflowId);
      ...
      return (
        <>
          ...
          <div
            className="mini-right-sidebar-content"
            style={{
              flex: 1,
              overflow: 'auto',
              columnCount: 2,
              ...
              >
                <Pagination className="w-full" style={{ width: '100%', display: 'flex', justifyContent: 'center' }}>
                  ...
                  >
                    ...
                    <PaginationItem>
                      <PaginationNext
                        onClick={() => setHistoryPage(Math.min(total, page + 1))}
                        style={{
    ...
    >
      ...
      >
        ...
        <div
          ...
          style={{ position: 'absolute', top: 0, left: -6, width: 12, height: '100%', cursor: 'col-resize', zIndex: 1 }}
        >
