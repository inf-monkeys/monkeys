...
interface IFieldNewDesignDisplayNameProps extends React.ComponentPropsWithoutRef<'div'> {
...
export const FieldNewDesignDisplayName: React.FC<IFieldNewDesignDisplayNameProps> = ({ form }) => {
  ...
  // 初始化 i18n 数据
  useEffect(() => {
    const currentValue = form.getValues('extraData.newDesignDisplayName');
    if (typeof currentValue === 'string') {
      // 将旧的字符串格式转换为 i18n 对象
      const i18nData = { [currentLanguageKey]: currentValue };
      setI18nDisplayName(i18nData);
      // 更新表单数据为 i18n 格式
      form.setValue('extraData.newDesignDisplayName', i18nData);
  ...
  const handleMainInputChange = (value: string) => {
    ...
    form.setValue('extraData.newDesignDisplayName', hasAnyValue ? newI18nData : {});
  };
  ...
  const handleSave = () => {
    ...
    form.setValue('extraData.newDesignDisplayName', hasAnyValue ? filteredData : {});
    setDialogOpen(false);
  };

  return (
    <FormField
      name="extraData.newDesignDisplayName"
      control={form.control}
      render={({ field }) => (
        <FormItem>
          <FormLabel>
            {t('workspace.flow-view.tooltip.more.association-editor.editor.field.new-design-display-name.label')}
          </FormLabel>
          <FormControl>
            <div className="relative">
              <Input
                placeholder={t(
                  'workspace.flow-view.tooltip.more.association-editor.editor.field.new-design-display-name.placeholder',
                )}
