import type { StoragePresignConfig } from '@/apis/common/typings';
import { vinesHeader } from '@/apis/utils';

const DEFAULT_EXPIRES_IN_SECONDS = 300;
const MAX_CONCURRENT_REQUESTS = 5;
const FETCH_PATCH_FLAG = '__monkeysOssPresignFetchPatched__';
...
let cachedConfig: StoragePresignConfig | null = null;
let configPromise: Promise<StoragePresignConfig | null> | null = null;
...
const sanitizePresignConfig = (config?: StoragePresignConfig | null): StoragePresignConfig | null => {
...
export const setPresignConfig = (config?: StoragePresignConfig | null) => {
...
const loadPresignConfig = async (): Promise<StoragePresignConfig | null> => {
  if (configLoaded) {
    return cachedConfig;
  }
  if (configPromise) {
...
const matchesPattern = (pattern: StoragePresignConfig['buckets'][number]['urlPatterns'][number], url: URL) => {
...
const shouldPresignUrlWithConfig = (value: string, config: StoragePresignConfig | null) => {
...
const buildAuthHeaders = () => {
  try {
    return vinesHeader({ useToast: false });
  } catch {
    return {};
  }
};
...
const requestPresignedUrl = async (url: string, config: StoragePresignConfig): Promise<string> => {
  const cacheKey = url;
...
export const initOssPresignInterceptors = (config?: StoragePresignConfig | null) => {
  if (config) {
